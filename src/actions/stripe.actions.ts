...
export const createProductPayement = async (name: string, description: string, price: number) => {
  try {
...
export async function createProductSubscription(name: string, description: string, amount: number, interval: 'week' | 'month' | 'year') {
  const product = await stripe.products.create({
    name,
    description,
  });
...
export const increaseReferrerBalance = async (referrerId: string, amount: number) => {
  try {
